...
#### 常用分片算法
1. 配置简单取模 -- PartitionByMod,可以用于分片列为整数类型的表，分片列mod分片基数，类全名：`io.mycat.route.function.PartitionByMod`
2. 配置哈希取模 -- PartitionByHashMod,用于多种数据类型如字符串，日期等，将username，goods_name等列进行hash计算并与count进行取模计算，类全名：`io.mycat.route.function.PartitionByHashMod`